  ...
  def successorStates(self, state):
    ...
    for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
      ...
      # Here's a code snippet for figuring out whether a new position hits a wall:
         #x,y = self.startingState()#currentPosition
         dx, dy = Actions.directionToVector(action)
         nextx,nexty = int(x + dx), int(y + dy)
         print type(nextx)
         print nexty
         hitsCorner = self.corners
         count = 0
         ...
         if not hitsWall:
          ...
          if nextFood not in corners_list:
           corners_list.append(list((nextx,nexty)))
           #print "breaking here", nextx ,nexty
           succ.append( ( ((nextx, nexty), nextFood), action, 1) )
          # print"or here "
          if nextFood in hitsCorner:
           succ.append( ( ((nextx, nexty), nextFood), action, 1) )
         #if not hitsCorner:
          #   nextFood = state[1].copy
           #  nextFood[nextx][nexty] = False
            # for corner in hitsCorner:
             #    if corner in hitsCorner:
              #       count = count + 1
               #      succ.append((((nextx, nexty), nextFood), action, 1))
